...
class Node {
   // Limite supérieure: Meilleur cas
// (Sac à dos fractionné)
    float ub;
 
    // Limite inférieure: Le pire des cas
// (0/1)
    float lb;
 
   // Niveau du noeud dans
// l'arbre de décision
    int level;
 
   // Stocke si le courant
// l'élément est sélectionné ou non
    boolean flag;
 // Valeur totale: Stocke le
// somme des valeurs de la
// articles inclus
    float tv;
 
    // Poids total: Stocke la somme de
// le poids des articles inclus
    float tw;
    public Node() {}
...
class Knapsack {
 ...
 // File d'attente prioritaire pour stocker des éléments
// basé sur les limites inférieures
        ...
        System.out.println("Articles pris"
                           + "dans le sac à dos sont");
        for (int i = 0; i < size; i++) {
            if (finalPath[i])
                System.out.print("1 ");
            else
    ...
    {
        //il faut essayer un exemple pour trouver la solution
